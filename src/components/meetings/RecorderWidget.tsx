...
export function RecorderWidget() {
  ...
  if (state.status === "idle")
    return error ? (
      <div
        role="alert"
        className="fixed bottom-5 right-5 z-[55] rounded border border-border bg-raised p-3"
      >
        {error}
  ...
  const silent =
    state.status === "recording" && state.silentSec >= SILENCE_WARN_SEC;
      ...
      {silent && (
        ...
        >
          ...
          <span>
            ...
            {state.deviceLabel || "the selected input"} for {state.silentSec}s.
            Check{" "}
            {isMacLike()
      ...
      <div className="flex items-center gap-3 px-3.5 py-2.5">
        ...
        {state.status === "recording" ? (
          <button
            ...
            className="rounded-md p-1.5 text-ink-2 hover:bg-hov hover:text-ink"
          >
            <Pause size={15} />
          </button>
        ) : (
...
export function SidebarRecordingDot() {
  ...
  return (
    <span className="ml-1 h-2 w-2 animate-pulse rounded-full bg-[var(--pal-red)]" />
  );
}
