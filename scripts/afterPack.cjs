// WHAT: electron-builder afterPack hook. On macOS, makes the bundled Convex
// backend binary executable and strips the quarantine flag from the packaged
// .app. On Windows, just verifies the .exe extraResource landed where expected.
...
// .app would be Gatekeeper-blocked when the app tries to spawn it. Windows has
// neither concept (no execute bit, no quarantine xattr) — that platform only
// needs a sanity check that the resource actually shipped.
